...
function nextErrorShotPath(label) {
  // Правка 1c-gui-test-kit (см. NOTICE.md): setErrorShotDir() зовёт только
  // cmdTest, поэтому у run/exec скриншот ошибки всегда падал в корень самого
  // навыка (skills/web-test/error-shot.png). Обёртка gui-test.ps1
  // складывает все артефакты прогона в свою папку и сообщает её путь агенту —
  // скриншот оставался единственным файлом снаружи, и в отчёте агента путь к
  // нему не совпадал с остальными артефактами. Переменная окружения не меняет
  // поведение по умолчанию (пусто — прежний единый файл).
  const dir = errorShotDir || process.env.WEB_TEST_ERROR_SHOT_DIR;
  if (!dir) return DEFAULT_ERROR_SHOT_PATH;
  return resolve(dir, `onec-error-${++errorShotSeq}-${slugify(label || 'shot')}.png`);
}
